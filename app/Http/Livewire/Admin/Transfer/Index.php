...
class Index extends Component
{
    ...
    public $search_name = "", $search_email = "", $search_mobile = "", $search_identifier = "",
        ...
        $search_status_id = "", $search_status = "", $search_refer, $search_bank_id = "",
        $search_price_from, $search_price_to, $search_created_from, $search_created_to;

    public $paymentMethods, $refers, $banks;
    ...
    {
        $query = Order::where('payment_type_id', 2)->orderBy('created_at', 'DESC');
        ...
        if ($this->search_bank_id  != '') {
            $query = $query->whereHas('transfer.bank', function($q){
                $q->where('id', $this->search_bank_id );
            });
            $this->resetPage();
        }
        ...
        return view('livewire.admin.transfer.index', compact('links', 'items'));
    }
}
