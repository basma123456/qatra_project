...
class Index extends Component
{
    ...
    {
        $marketersIds = Marketer::where('marketer_admin_id' , auth()->guard('marketer_admin')->id())->pluck('id')->toArray();
        $orders = Order::whereIn('marketer_id' , $marketersIds )->findMany($this->mySelected);
    ...
    {
//        $orders = Order::where('marketer_id' , auth()->guard('marketer')->id())->findMany($this->mySelected);
        $marketersIds = Marketer::where('marketer_admin_id' , auth()->guard('marketer_admin')->id())->pluck('id')->toArray();
        $orders = Order::whereIn('marketer_id' , $marketersIds )->findMany($this->mySelected);
...
//        $orders = Order::where('marketer_id' , auth()->guard('marketer')->id())->findMany($this->mySelected);
    ...
    {
//        $items = Order::where('marketer_id' , auth()->guard('marketer')->id())->findMany($this->mySelected);
        $marketersIds = Marketer::where('marketer_admin_id' , auth()->guard('marketer_admin')->id())->pluck('id')->toArray();
        $items = Order::whereIn('marketer_id' , $marketersIds )->findMany($this->mySelected);
...
//        $items = Order::where('marketer_id' , auth()->guard('marketer')->id())->findMany($this->mySelected);
    ...
    {
//        $query = Order::where('marketer_id' , auth()->guard('marketer')->id())->orderBy('created_at', 'DESC');
        $marketersIds = Marketer::where('marketer_admin_id' , auth()->guard('marketer_admin')->id())->pluck('id')->toArray();
        $query = Order::whereIn('marketer_id' , $marketersIds )->orderBy('created_at', 'DESC');
        ...
        return view('livewire.marketer-admin.orders.index', compact('links', 'items'));
    }
