            ...
            <div class="card-body  search-group">
                <div class="row">
                    <div class="col-md-12 text-end mb-2">
                        <a href="{{ route('admin.slider.create') }}" class="btn btn-outline-success btn-sm">اضافة</a>
                    </div>
                </div>
                <form action="{{route('admin.slider.index')}}" method="get">
                    <div class="row">
                        <div class="col-md-3 mb-2">
                            <input type="text" value="" name="title" placeholder="ابحث بالعنوان" class="form-control">
                        </div>
                        {{-- <div class="col-md-3">
                                ...
                                <tr class="bluck-actions" style="display: none" scope="row">
                                    <td colspan="8">
                                        <div class="col-md-12 mt-0 mb-0 text-center" >
                                            <button form="update-pages" class="btn btn-success btn-sm" type="submit" name ="publish" value="1" title="نشط"> <i class="fa fa-check"></i></button>
                                            <button form="update-pages" class="btn btn-warning btn-sm" type="submit" name ="unpublish" value="1" title="غير نشط">  <i class="fa fa-ban"></i></button>
                                            <button form="update-pages" class="btn btn-danger btn-sm" type="submit" name ="delete_all" value="1" title="الغاء الكل">  <i class="fas fa-trash-alt"></i></button>
                                        </div>
                                    </td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center">
                                                @if($slider->status == 1)
                                                    <a href="{{ route('admin.slider.update-status', $slider->id )}}" class="btn btn-xs btn-success btn-sm m-1" title="نشط"><i class="fa fa-check"></i></a>
                                                @else
                                                    <a href="{{ route('admin.slider.update-status', $slider->id )}}" class="btn btn-xs btn-outline-secondary btn-sm m-1" title="غير نشط"><i class="fa fa-ban"></i></a>
                                                @endif
                                                <a href="{{ route('admin.slider.edit',$slider->id) }}" class="btn btn-outline-primary btn-sm m-1" title="تعديل"><i class="fas fa-pencil-alt"></i></a>
                                                <a href="{{ route('admin.slider.show', $slider->id) }}" class="btn btn-xs btn-outline-info btn-sm m-1" title="عرض"><i class="fas fa-eye"></i></a>
                                                <a class="btn btn-outline-danger btn-sm m-1" data-bs-toggle="modal" data-bs-target="#exampleModal{{ $slider->id }}" title="الغاء">
