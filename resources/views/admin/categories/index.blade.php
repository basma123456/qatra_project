...
@section('name_ar',  "الاقسام")
@section('name_ar_page',  "الاقسام" )
            ...
            <div class="card-body  search-group">
                <div class="row">
                    <div class="col-md-12 text-end mb-2">
                        <a href="{{ route('admin.categories.create') }}" class="btn btn-outline-success btn-sm">اضافة</a>
                    </div>
                </div>
                {{-- Start Form search --}}
                <form action="{{route('admin.categories.index')}}" method="get">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-3 mb-2">
                            <input type="test"  value="{{ old('description', request()->input('description')) }}" name="description" placeholder="ابحث بالوصف" class="form-control">
                        </div>
                        <div class="col-md-3 mb-2">
                        ...
                        <div class="search-input col-md-2">
                            ...
                            <a class="btn btn-success btn-sm" href="{{route('admin.categories.index')}}"><i class="refresh ion ion-md-refresh"></i></a>
                        </div>
                    </div>
                </form>
                {{-- End Form search --}}
...
{{--                                        {{ substr(removeHTML($item->description_ar),0,30)   }}--}}
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center">
                                            @if($item->status == 1)
                                                <a href="{{ route('admin.categories.update-status', $item->id )}}" name="@lang('admin.active')" class="btn btn-xs btn-success btn-sm m-1"><i class="fa fa-check"></i></a>
                                            @else
                                                <a href="{{ route('admin.categories.update-status', $item->id )}}" name="@lang('admin.dis_active')"  class="btn btn-xs btn-outline-secondary btn-sm m-1"><i class="fa fa-ban"></i></a>
                                            @endif

                                            @if($item->feature == 1)
                                                <a href="{{ route('admin.categories.update-featured', $item->id )}}" name="@lang('admin.feature')"  class="btn btn-xs btn-warning btn-sm m-1"><i class="fa fa-star"></i></a>
                                            @else
                                                <a href="{{ route('admin.categories.update-featured', $item->id )}}" name="@lang('admin.feature')"  class="btn btn-xs btn-outline-secondary btn-sm m-1"><i class="fa fa-star"></i></a>
                                            @endif



                                            <a href="{{ route('admin.categories.show', $item->id) }}" name="@lang('admin.show')" class="btn btn-xs btn-outline-info btn-sm m-1"><i class="fas fa-eye"></i></a>


                                            <a href="{{ route('admin.categories.edit',$item->id) }}" name="@lang('admin.edit')" class="btn btn-outline-primary btn-sm m-1"><i class="fas fa-pencil-alt"></i></a>
