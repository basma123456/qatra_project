...
@section('title_page', trans('admin.cities'))
@section('title_route', route('admin.cities.index') )
@section('button_page')
<a href="{{ route('admin.cities.create') }}" class="btn btn-outline-success btn-sm">@lang('admin.create')</a>
@endsection
    ...
    <div class="card-body  search-group">
        <div class="row">
            <div class="col-md-12 text-end mb-2">
                <a href="{{ route('admin.cities.create') }}" class="btn btn-outline-success btn-sm">اضف مدينه</a>
            </div>
        </div>
        ...
        <form action="{{route('admin.cities.index')}}" method="get">
            <div class="row">
                ...
                <div class="search-input col-md-2">
                    ...
                    <a class="btn btn-success btn-sm" href="{{route('admin.cities.index')}}" data-hover="{{ trans('button.reset') }}"> <i class="fas fa-undo"></i> </a>
                </div>
            </div>
        </form>
    </div>
...
<div class="container-fluid">
    <div class="card">
        <div class="card-body">
            <form id="update-cities" action="{{route('admin.cities.actions')}}" method="post">
                @csrf
            </form>
            <div class="table-responsive">
                <table id="main-datatable" class="table table-bordered text-center table-striped table-table-success table-hover" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-center">
                                    @if($item->status == 1)
                                    <a href="{{ route('admin.cities.update-status', $item->id) }}" class="btn btn-xs btn-outline-success btn-sm m-1" data-hover="{{ trans('button.active') }}"> <i class="fas fa-solid fa-check"></i> </a>
                                    @else
                                    <a href="{{ route('admin.cities.update-status', $item->id) }}" class="btn btn-xs btn-outline-warning btn-sm m-1" data-hover="{{ trans('button.unactive') }}"> <i class="fas fa-solid fa-ban"></i> </a>
                                    @endif

                                    
                                    <a href="{{ route('admin.cities.show', $item->id) }}" name="@lang('admin.show')" class="btn btn-xs btn-outline-info btn-sm m-1"><i class="fas fa-eye"></i></a>


                                    <a href="{{ route('admin.cities.edit',$item->id) }}" name="@lang('admin.edit')" class="btn btn-outline-primary btn-sm m-1"><i class="fas fa-pencil-alt"></i></a>
                        ...
                        @include('admin.layouts.delete', ['route'=> 'admin.cities.destroy'])
