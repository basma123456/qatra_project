            ...
            <div class="card-body  search-group">
                <form wire:submit.prevent="filters">
                    <div class="row" wire:ignore>
                        ...
                        <div class="col-md-2 ">
                            <select wire:model="search_bank_id" class="form-control">
                                <option value=""> @lang('Bank') </option>
                ...
                <table id="main-datatable" class="table table-striped table-bordered " style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        ...
                        <tr class="bluck-actions bulk-action-order" @if (empty($mySelected)) style="display: none" @endif scope="row">
                            <td colspan="16">
                                <div class="col-md-12 mt-0 mb-0 text-center">
                                    ...
                                    <button wire:click.prevent="unpublishSelected" @if (empty($mySelected)) disabled @endif class="btn btn-warning btn-sm my-2" type="submit">
                                        {{ @$this->orderStatus->where('id', 301)->first()->name }}
                                    </button>
                                    <button wire:click.prevent="publishSelected" @if (empty($mySelected)) disabled @endif class="btn btn-success btn-sm my-2" type="submit">
                                        {{ @$orderStatus->where('id', 100)->first()->name }}
                                    </button>
                                    <button wire:click.prevent="cancelSelected" @if (empty($mySelected)) disabled @endif class="btn btn-outline-danger  btn-sm my-2" type="submit">
                                        {{ @$orderStatus->where('id', 404)->first()->name }}
                                    </button>
                    ...
                    <tbody>
                        ...
                        <tr class="text-center">
                            ...
                            <td>{{ $item->id }}</td>
                            <td>                        
                                <span class="btn-sm py-0">{{ @$item->transfer?->bank?->name }}</span>
                            </td>
                            <td>{{ $item->amount }}</td>
                            <td>{{ $item->tax }}</td>
                            <td>{{ $item->total }}</td>
                            <td>
                                <a href="{{ asset('storage/'. $item->transfer->transfer_img) }}" class="btn btn-sm btn-primary" target="_blank" >
                                    صوره التحميل
                                </a>
                            </td>
                            ...
                            <td>
                                @livewire('admin.orders.show', ['order_id' => $item->id], key($item->id))
                            </td>
                            <td>
